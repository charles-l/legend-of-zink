...
if os.path.exists(json_file):
    ...
    if d:
        ...
        included_tiles = {(tile['rect'][0] // tile_size, tile['rect'][1] // tile_size): tile['collision'] for tile in d}
...
while not rl.window_should_close():
    ...
    if rl.gui_value_box(rl.Rectangle(60, 10, 100, 30), "tile size", tile_size_ptr, 1, 255, tile_size_edit):
    ...
    for x in range(tex.width // tile_size + 1):
        rl.draw_line(x * tile_size, 0,
                     x * tile_size, tex.height,
                     rl.PURPLE)

    for y in range(tex.height // tile_size + 1):
        rl.draw_line(0, y * tile_size,
                     tex.width, y * tile_size,
                     rl.PURPLE)

    for tile in included_tiles:
        color = rl.RED if included_tiles[tile] == 'collide' else rl.WHITE
        rl.draw_rectangle_lines(tile[0] * tile_size, tile[1] * tile_size, tile_size, tile_size, color)

    mouse_pos_world = rl.get_screen_to_world_2d(rl.get_mouse_position(), camera)
    mouse_tile = (int(mouse_pos_world.x // tile_size), int(mouse_pos_world.y // tile_size))
...
with open(json_file, 'w') as f:
    json.dump([
        {"rect": [tile[0] * tile_size, tile[1] * tile_size, tile_size, tile_size],
         "collision": included_tiles[tile]}
        for tile in included_tiles], f)
